...
const DetailDiary = () => {
  ...
  const delPopupRef = useRef();

  useGSAP(() => {
    if (delPopup) {
      gsap.from(delPopupRef.current, {
        opacity: 0,
        scale: 0.9,
        y: -10,
        duration: 0.2,
  ...
  if (loading) {
    return (
      <section className="flex flex-col items-center justify-center h-dvh">
        <svg
          className="p-2 duration-500 bg-indigo-400 rounded-full animate-spin dark:bg-indigo-700 size-14"
          width="33"
          height="32"
          viewBox="0 0 33 32"
          fill="none"
      ...
      <div className="relative max-w-5xl px-3 py-8 mx-auto border-2 shadow-lg sm:px-6 md:px-10 bg-primary dark:bg-gray-900 rounded-2xl md:mt-8">
        ...
        <div className="hidden gap-5 bg-transparent other-btns md:flex md:flex-row md:justify-between md:mb-8 lg:justify-end xl:absolute xl:-right-16 xl:top-20 xl:flex xl:items-center xl:justify-center xl:flex-col">
          <PDFDownloadLink
            ...
            className="inline-flex p-2 transition-colors duration-500 bg-indigo-400 diary-animation rounded-xl text-primary hover:bg-indigo-500 dark:bg-indigo-700 dark:hover:bg-indigo-400"
          >
            <svg
              viewBox="0 0 24 24"
              xmlns="http://www.w3.org/2000/svg"
          ...
          <button
            className="inline-flex p-2 transition-colors duration-500 bg-red-400 diary-animation rounded-xl text-primary hover:bg-red-500 dark:bg-red-700 dark:hover:bg-red-400"
        ...
        <div className="flex items-center justify-between mb-5">
          <div className="inline-block px-2 sm:px-3 py-0 text-[0.6rem] sm:text-[0.65rem] md:text-xs font-light tracking-widest bg-indigo-100 rounded-full dark:bg-indigo-700 font-unbounded">
        ...
        <div className="flex items-center justify-between">
          <div className="flex flex-col items-start w-full md:w-4/5">
            <textarea
              ...
              autoComplete="true"
              rows={1}
              className="w-full p-1 overflow-hidden text-2xl font-medium bg-transparent border-b-2 border-gray-300 border-solid resize-none sm:p-2 md:text-3xl font-unbounded focus:outline-none whitespace-nowrap"
            >
              {values.heading}
            </textarea>
            <h2 className="error">{hedingError}</h2>
          </div>
          <div className="flex-col items-center hidden w-1/5 text-center md:flex">
            <button
        ...
        <h4 className="text-center error">{updateError}</h4>

        {/* mobile print, delete and save buttton */}
        <div className="flex items-center justify-between mb-8">
          <div className="flex md:hidden">
            <PDFDownloadLink
              ...
              fileName={values.heading + "-Inkwell"}
              className="inline-flex h-10 p-2 mx-3 transition-colors duration-500 bg-indigo-400 diary-animation rounded-xl text-primary hover:bg-indigo-500 dark:bg-indigo-700 dark:hover:bg-indigo-400"
            >
              <svg
                viewBox="0 0 24 24"
                xmlns="http://www.w3.org/2000/svg"
            ...
            <button
              className="inline-flex h-10 p-2 transition-colors duration-500 bg-red-400 diary-animation rounded-xl text-primary hover:bg-red-500 dark:bg-red-700 dark:hover:bg-red-400"
              onClick={() => setDelPopup(true)}
            >
              <svg
                viewBox="0 0 24 24"
          ...
          <div className="flex w-2/5 sm:w-1/3 md:hidden">
            <button
              className={`cursor-pointer mx-auto h-10 bg-indigo-400 dark:bg-indigo-700 font-semibold text-base text-primary 
                  hover:bg-indigo-500 transition-all 
                  ${
                    isSaving
                      ? "w-11 h-11 rounded-full duration-500"
                      : "w-full rounded-lg duration-500"
                  } ${allowSave ? "" : "notallowed"}`}
              onClick={handleSaveClick}
            >
              <svg
        ...
        <div className="relative mt-5">
          <div className="flex flex-wrap items-center justify-center">
            {tags.map((tag, index) => (
              <span
                key={index}
                className={`inline-flex items-center ${colors[index]} px-3 py-[2px] rounded-full font-semibold text-xs sm:text-sm md:text-base inline m-1`}
              >
                #{tag}
                <button
        ...
        <div className="mt-8 editor">
          <ReactQuill
            ...
            onChange={(content) => {
              ...
              // Inline sanitization: Remove HTML tags and trim whitespace to get perfect word count
      ...
      {delPopup && (
        <div className="popup fixed h-dvh w-full top-0 left-0 bg-[#7b7f83a3] dark:bg-[#191a1ba3] z-50 p-3 flex items-end lg:items-center">
          ...
          >
            ...
            <p className="font-medium text-center font-nunito text-secondary dark:text-darkPrimary mb-7">
